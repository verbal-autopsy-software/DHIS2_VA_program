...
class VerbalAutopsyEvent(object):
    ...
    def __init__(self, va_id, program, orgunit, icd10, algorithm_metadata, file_id):
        ...
        self.attributes =  [
            {"attribute": "XSFOyybvYJ9", "value": self.random_sex()}
        ]
        ...
        self.enrollments = [
            {"program": self.program, "orgUnit": self.orgunit, "events": self.events}
        ]
        ...
        if age_choice == 'birth_date':
            self.attributes.append({"attribute": "P1xsdeFzhCb", "value": birth_date_formatted})
        elif age_choice == 'age':
    ...
    def format_to_dhis2(self):
        """
        Format object to DHIS2 compatible tracked rntity instance for DHIS2 API
        :rtype: dict
        """
        tei = {
...
if __name__ == '__main__':
    ...
    print("{}+++ Warning: This script does load {} random Verbal Autopsy events to {} "
          "and posts SQlite files to /api/30/fileResources. OK? Abort with CTRL+C {}".format(WARNING, args.events, args.server, ENDC))
    try:
        time.sleep(3)
    ...
    for i in range(args.events):
        ...
        trackedEntityInstances.append(e.format_to_dhis2())
        print("{} out of {} events processed".format(i + 1, args.events))

    export['trackedEntityInstances'] = trackedEntityInstances

    with open('trackedEntityInstances.json', 'w') as json_file:
        json.dump(export, json_file, indent=4)

    print("Import file stored at trackedEntityInstances.json - {} Proceeding to POST that import file NOW...{} Abort with CTRL+C.".format(WARNING, ENDC))
    try:
        time.sleep(5)
    except KeyboardInterrupt:
        print("POSTing events aborted. Nothing was imported to {}".format(args.server))

    log = api.post('trackedEntityInstances', data=export)
    print("\n\n")
