...
Example request:
`curl -X POST -v -F "file=@/path/to/verbalautopsy-raw-someid.db;filename=someid.db" https://va.swisstph-mis.ch/api/fileResources`
...
`curl -X POST "https://va.swisstph-mis.ch/api/events" -H "Content-Type: application/json" -u admin:district -d @event.json`
